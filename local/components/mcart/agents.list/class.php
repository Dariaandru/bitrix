...
class AgentsList extends CBitrixComponent implements Controllerable, Errorable
{
    ...
    final public function onPrepareComponentParams($arParams): array
    {
        ...
        if (!isset($arParams['ELEMENTS_COUNT']) || intval($arParams['ELEMENTS_COUNT']) <= 0) {
            $arParams['ELEMENTS_COUNT'] = 20; // Дефолтное значение - 20 элементов
        }
...
$nav->allowAllRecords(true)
    ...
    'limit' => $nav->getLimit(),
        ...
        while ($arAgent = $rsAgents->fetch()) {
             ...
             */
             ...
             // Process type
             if (!empty($arAgent['UF_ACTIVITY'])) {
                $enumEntity = \CUserFieldEnum::GetList([], ["ID" => $arAgent['UF_ACTIVITY']]);
                if ($enum = $enumEntity->GetNext()) {
                    $arAgent['UF_ACTIVITY'] = $enum["VALUE"];
                }
            }
    ...
    public function clickStarAction($agentID)
    {
         ...
         */
         ...
         $currentValue = CUserOptions::GetOption('mcart_agent', 'options_agents_star');
    
    if ($currentValue) {
        $value = is_array($currentValue) ? $currentValue : [$currentValue];
